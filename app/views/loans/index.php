...
<div class="loan-summary-card"><div class="loan-summary-title">Total Loans</div><div class="loan-summary-value"><?=number_format($totalLoans)?></div></div>
<div class="loan-summary-card"><div class="loan-summary-title">Active Loans</div><div class="loan-summary-value"><?=number_format($activeLoans)?></div></div>
<div class="loan-summary-card"><div class="loan-summary-title">Total Principal</div><div class="loan-summary-value">₱<?=number_format($totalPrincipal,2)?></div></div>
<div class="loan-summary-card"><div class="loan-summary-title">Total Payable</div><div class="loan-summary-value">₱<?=number_format($totalPayable,2)?></div></div>
</div>
...
<p>You haven't created any loans yet.</p><br>
...
<form method="POST" action="index.php?url=loans/delete" onsubmit="return confirm('Are you sure you want to cancel this loan?')">
<input type="hidden" name="id" value="<?=$loanId?>">
<button type="submit" class="loan-action-item loan-action-danger"><span>✕</span> Cancel</button>
</form>
<?php endif; ?>
...
<div><h2>Edit Loan</h2><p>Update the loan information.</p></div>
...
<div class="form-group"><label>Borrower</label><input type="text" id="edit_borrower" readonly></div>
<div class="form-group"><label>Loan Category</label><input type="text" id="edit_category" readonly></div>

<div class="form-group"><label>Principal Amount</label><input type="number" id="edit_principal_amount" name="principal_amount" min=".01" step=".01" required></div>
<div class="form-group"><label>Interest Rate (%)</label><input type="number" id="edit_interest_rate" name="interest_rate" min="0" step=".01" required></div>
...
<div class="form-group"><label>Term</label><input type="number" id="edit_term" name="term" min="1" required></div>
...
<div class="form-group"><label>Processing Fee</label><input type="number" id="edit_processing_fee" name="processing_fee" min="0" step=".01"></div>
<div class="form-group"><label>Release Date</label><input type="date" id="edit_release_date" name="release_date"></div>
<div class="form-group"><label>First Payment Date</label><input type="date" id="edit_first_payment_date" name="first_payment_date"></div>

<div class="form-group loan-form-full"><label>Purpose</label><input type="text" id="edit_purpose" name="purpose" maxlength="255"></div>
<div class="form-group loan-form-full"><label>Notes</label><textarea id="edit_notes" name="notes" rows="4"></textarea></div>
...
<div class="modal-header">
<div><h2>Create Loan</h2><p>Create a new borrower loan.</p></div>
...
<div class="form-group"><label>Principal Amount</label><input type="number" id="principal_amount" name="principal_amount" min=".01" step=".01" required></div>
<div class="form-group"><label>Interest Rate (%)</label><input type="number" id="interest_rate" name="interest_rate" min="0" step=".01" value="0.00" required></div>
...
<div class="form-group"><label>Term</label><input type="number" id="term" name="term" min="1" value="1" required></div>
...
<div class="form-group"><label>Processing Fee</label><input type="number" id="processing_fee" name="processing_fee" min="0" step=".01" value="0.00"></div>
...
<div class="form-group loan-form-full"><label>Purpose</label><input type="text" id="purpose" name="purpose" maxlength="255" placeholder="Example: Business capital"></div>
<div class="form-group loan-form-full"><label>Notes</label><textarea id="notes" name="notes" rows="4" placeholder="Additional notes..."></textarea></div>
...
<div class="modal-header">
<div><h2>Loan Details</h2><p>Complete information about this loan.</p></div>
...
function formatPaymentMethod(v){v=String(v||'installment').toLowerCase();return v==='full_payment'?'Full Payment':v==='installment'?'Installment':formatText(v)}
function formatMoney(v){return'₱'+Number(v||0).toLocaleString('en-PH',{minimumFractionDigits:2,maximumFractionDigits:2})}
function formatText(v){if(!v)return'-';return String(v).replace(/_/g,' ').replace(/\b\w/g,x=>x.toUpperCase())}
function escapeHtml(v){return String(v??'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#039;')}
function createStatusBadge(v){v=String(v||'pending').toLowerCase().trim();return`<span class="loan-status loan-status-${escapeHtml(v)}">${escapeHtml(formatText(v))}</span>`}

function openCreateLoanModal(){document.getElementById('createLoanModal')?.classList.add('active')}
function closeCreateLoanModal(e){if(e&&e.target!==e.currentTarget)return;document.getElementById('createLoanModal')?.classList.remove('active')}
function closeEditLoanModal(e){if(e&&e.target!==e.currentTarget)return;document.getElementById('editLoanModal')?.classList.remove('active')}
function closeLoanDetails(e){if(e&&e.target!==e.currentTarget)return;document.getElementById('loanDetailsModal')?.classList.remove('active')}
function closePenaltyModal(e){if(e&&e.target!==e.currentTarget)return;document.getElementById('penaltyModal')?.classList.remove('active')}
...
editLoanForm?.addEventListener('submit',e=>{
    ...
    if(!confirm('Are you sure you want to save these changes?'))e.preventDefault();
});
...
function formatDateForInput(d){return[d.getFullYear(),String(d.getMonth()+1).padStart(2,'0'),String(d.getDate()).padStart(2,'0')].join('-')}
...
function formatDisplayDate(d){return d?d.toLocaleDateString('en-PH',{year:'numeric',month:'long',day:'numeric'}):'-'}
...
function openLoanDetails(id,loanNumber,borrower,category,principal,interestRate,interestType,term,termPeriod,paymentType,fee,totalInterest,totalPayable,releaseDate,firstPaymentDate,status,purpose,notes){
    ...
    let due=firstPaymentDate?parseLocalDate(firstPaymentDate):null;

    if(!due&&releaseDate){
        const release=parseLocalDate(releaseDate);
        if(release)due=addPaymentPeriod(release,termPeriod,1);
    }

    const dueElement=document.getElementById('detail_due_date');
    if(dueElement)dueElement.innerHTML=due?'<span class="loan-due-date">'+escapeHtml(formatDisplayDate(due))+'</span>':'-';

    set('detail_release_date',releaseDate?formatDisplayDate(parseLocalDate(releaseDate)):'-');
    set('detail_first_payment_date',firstPaymentDate?formatDisplayDate(parseLocalDate(firstPaymentDate)):due?formatDisplayDate(due):'-');
...
document.getElementById('approveLoanForm')?.addEventListener('submit',e=>{
    if(!document.getElementById('approve_loan_id')?.value){
        e.preventDefault();
        alert('No loan selected.');
    }else if(!confirm('Are you sure you want to approve this loan?\n\nOnce approved, the loan will proceed to the next stage.'))e.preventDefault();
});
