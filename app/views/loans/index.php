...
|--------------------------------------------------------------------------
|
| The loan's original total_payable remains unchanged in the database.
| We subtract all payments made against the loan only for display.
|
| Example:
|
| Original Total Payable = ₱5,000
| Payments Made         = ₱2,000
| Remaining Payable     = ₱3,000
|
| Fully Paid:
|
| Original Total Payable = ₱4,433
| Payments Made         = ₱4,433
| Remaining Payable     = ₱0
|
|--------------------------------------------------------------------------
...
foreach($loans as $loan){
    ...
    /*
     * If the controller/model already provides a paid amount,
     * use it when available.
     */
    if(array_key_exists('total_paid',$loan)){
        $loanPaymentTotals[$loanId]=(float)$loan['total_paid'];
    }elseif(array_key_exists('paid_amount',$loan)){
        $loanPaymentTotals[$loanId]=(float)$loan['paid_amount'];
    }else{
        $loanPaymentTotals[$loanId]=0;
    }
}
...
|--------------------------------------------------------------------------
|
| If the controller already provides payments for each loan, this will
| also detect common payment field names.
|
|--------------------------------------------------------------------------
*/
...
foreach($loans as $loan){
    ...
    if(!empty($loan['payments']) && is_array($loan['payments'])){
        ...
        foreach($loan['payments'] as $payment){
            ...
            $paymentTotal+=(float)$paymentAmount;
        }

        $loanPaymentTotals[$loanId]=$paymentTotal;
    }
}
...
.loan-decision-reject:hover{
    background:#fecaca;
    box-shadow:0 0 20px rgba(239,68,68,.65);
    transform:translateY(-1px)
}
...
<span class="badge">
<?=htmlspecialchars($tenantRole??'User')?>
</span>
...
$paidAmount=
    (float)($loanPaymentTotals[$loanId]??0);
...
<span
    class="loan-status loan-status-<?=htmlspecialchars($status)?>"
>
...
<div
    class="modal-overlay"
    id="editLoanModal"
    onclick="closeEditLoanModal(event)"
>
...
<span class="account-balance-hint">
...
<div
    class="loan-detail-item <?=$id==='notes'?'loan-detail-full':''?>"
>
...
<span class="penalty-summary-label">
Loan Number
</span>

<span
    class="penalty-summary-value"
    id="penalty_loan_number"
>
-
</span>
...
<form
    method="POST"
    action="index.php?url=loans/penalty"
    id="penaltyForm"
>
...
<input
    type="number"
    name="schedule_id"
    id="penalty_schedule_id"
    min="1"
    required
    placeholder="Enter schedule ID"
>

<span class="penalty-calculation-hint">
The penalty must be attached to a specific loan schedule.
</span>
...
<span
    class="penalty-calculation-hint"
    id="penaltyRateHint"
>
Enter the fixed penalty amount.
</span>
...
<span class="penalty-total-label">
Calculated Penalty
</span>

<span
    class="penalty-total-value"
    id="penalty_total_display"
>
₱0.00
</span>
...
function openCreateLoanModal(){
    ...
    document
        .getElementById('createLoanModal')
...
function closeLoanDetails(e){
    ...
    document
        .getElementById('loanDetailsModal')
        ?.classList.remove('active');
...
const penaltyTypeInput=
    document.getElementById(
        'penalty_type'
    );

const penaltyRateInput=
    document.getElementById(
        'penalty_rate'
    );

const penaltyBaseInput=
    document.getElementById(
        'penalty_base_amount'
    );
...
const penaltyTotalDisplay=
    document.getElementById(
        'penalty_total_display'
    );

const penaltyRateHint=
    document.getElementById(
        'penaltyRateHint'
    );
...
function updatePenaltyCalculation(){
    ...
    const type=
        String(
            penaltyTypeInput.value||'fixed'
        ).toLowerCase();

    const rate=
        Number(
            penaltyRateInput.value||0
        );

    const base=
        Number(
            penaltyBaseInput.value||0
        );
    ...
    if(type==='percentage'){
        ...
        penaltyRateHint.textContent=
            'Percentage penalty. Days overdue can be handled by the backend.';
        ...
        penaltyRateHint.textContent=
            'Fixed penalty amount per overdue day.';
    ...
    if(penaltyTotalDisplay){
        ...
        penaltyTotalDisplay.textContent=
            formatMoney(amount);
...
penaltyTypeInput?.addEventListener(
    'change',
    updatePenaltyCalculation
);

penaltyRateInput?.addEventListener(
    'input',
    updatePenaltyCalculation
);

penaltyBaseInput?.addEventListener(
    'input',
    updatePenaltyCalculation
);
    ...
    ?.addEventListener(
        ...
        e=>{
            ...
            if(!scheduleId?.value){
                ...
                alert(
                    'Please enter the loan schedule ID.'
                );
            ...
            if(!reason?.value.trim()){
                ...
                alert(
                    'Please enter the reason for the penalty.'
                );
            ...
            if(
                !confirm(
                    'Are you sure you want to apply this penalty?'
                )
            ){
...
document
    .getElementById('approveLoanForm')
    ?.addEventListener(
        'submit',
        e=>{
